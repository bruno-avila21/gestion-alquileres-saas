...
public class Phase6ApiFactory : WebApplicationFactory<Program>
{
    public string DbName { get; } = "Phase6TestsDb_" + Guid.NewGuid();
    ...
    public async Task<(Guid contractId, Guid tenantId, HttpClient client)> SetupContractWithTenantAsync(string slug)
    {
        var c = await AuthedClientAsync(slug);

        var propResp = await c.PostAsJsonAsync("/api/v1/properties", new
        {
            address = "Lavalle 400", city = "CABA", province = "CABA",
        ...
        {
            firstName = "Laura", lastName = "Gómez", dni = "30123456",
            email = $"laura@{slug}.com", phone = (string?)null,
        });
